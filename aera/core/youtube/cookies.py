...
class CookieManager:
    ...
    def get_cookies(self):
        if not self.checked:
            if os.path.exists("aera/cookies"):
                for file in os.listdir("aera/cookies"):
        ...
        if not self.cookies:
            ...
            return None
        return f"aera/cookies/{random.choice(self.cookies)}"

    async def save_cookies(self, urls: list[str]) -> None:
        logger.info("🍪 Saving cookies from urls...")
        saved_count = 0
        os.makedirs("aera/cookies", exist_ok=True)
        for url in urls:
            try:
                path = f"aera/cookies/cookie{random.randint(10000, 99999)}.txt"
